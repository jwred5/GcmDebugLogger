...
1) Put your Sender ID in the GcmRegistration.SENDER_ID constant
2) If you are using a self-signed certificate, put it in a BKS keystore and place the keystore in the res/raw folder overwriting 'cacerts'.  Otherwise, set MainActivity.SELF_SIGNED_DESTINATION to false.
3) Put the password for the keystore in MainActivity.KEYSTORE_PASSWORD
4) Put the URL of where to send the POST request in MainActivity.API_LOCATION